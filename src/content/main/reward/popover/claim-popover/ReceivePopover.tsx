...
const ReceivePopover = (props: ParentProps) => {
  ...
  const { claimChestSMTReward, claimSurpriseSMTReward, isLoading } =
    useRewards();

  const onHandleProcess = async () => {
    if (props.rewardTitle === 'chest') await claimChestSMTReward(props.amount);
    else await claimSurpriseSMTReward(props.amount);
    props.onHandleClaimNext('sure', props.amount);
  };
